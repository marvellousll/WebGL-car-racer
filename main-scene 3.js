      ...
      this.materials = { plastic: new Material( phong_shader, 
                   ...
                   plastic_stars: new Material( texture_shader_2,    
                           ...
                           metal: new Material( phong_shader,
                                    { ambient: 0.5 , diffusivity: 1, specularity: 1, color: Color.of( 1,.5,1,1 ) } ),
...
//                                       ambient: 0.6, diffusivity: 0, specularity: 0, color: Color.of(.4,.4,.4,1) }),
      ...
      this.player_color = Color.of( .5,.5,0,1 );
  ...
  make_control_panel()
    {                                 // make_control_panel(): Sets up a panel of interactive HTML elements, including
      ...
      this.key_triggered_button( "Reverse",     [ "r" ], () => this.acceleration = -0.03, undefined, () => this.acceleration = 0.01 );
      this.key_triggered_button( "Accelerate",[ "w" ], () => this.acceleration = 0.07, undefined, () => this.acceleration = -0.01 );
      this.new_line();
      this.key_triggered_button( "Move Left",   [ "d" ], () => this.thrust[0] =  -1, undefined, () => this.thrust[0] = 0 );
      //this.key_triggered_button( "Back",   [ "s" ], () => this.thrust[2] = -1, undefined, () => this.thrust[2] = 0 );
      this.key_triggered_button( "Move Right",  [ "f" ], () => this.thrust[0] = 1, undefined, () => this.thrust[0] = 0 );
      
      this.key_triggered_button( "Turn Right",  [ "s" ], () => this.bodies[0].drawn_location.post_multiply(Mat4.scale([2,2,1])).post_multiply(Mat4.rotation(-0.05,Vec.of(0,1,0))).post_multiply(Mat4.scale([0.5,0.5,1])), undefined, () => this.model_transform = this.model_transform );
      ...
      this.key_triggered_button( "Start",  [ "f" ], () => this.game_start = 1);
      this.key_triggered_button( "TPP/FPP",  [ "p" ], () => this.perspective = !this.perspective);
      this.key_triggered_button( "Overlook",  [ "o" ], () => this.overlook = 1);
    }
    ...
    {    
      if( !context.scratchpad.controls ) 
        {     
          ...
          program_state.set_camera( Mat4.look_at( Vec.of( 90,90,25 ), Vec.of( 90,90,0 ), Vec.of( 0,1,0 ) ) );
...
//             this.perspective = 0;
      ...
      let intro_tranform = Mat4.identity().times(Mat4.translation([90,90,0]))
                                          .times(Mat4.scale([20,20,1/5]));
      this.shapes.box.draw(context,program_state,intro_tranform,this.materials.plastic);
      ...
      //console.log(this.bodies[0].drawn_location);
      if(this.overlook){
          const desired_camera = Mat4.inverse( Mat4.identity().times( Mat4.translation( [ 0,140,25 ] ) ).times(Mat4.rotation(-0.5*Math.PI,Vec.of(1,0,0))) );
        ...
        if(!this.perspective){
          const desired_camera = Mat4.inverse( this.bodies[0].drawn_location.times( Mat4.translation( [ 0,5,10 ] ) ).times(Mat4.rotation(-0.3,Vec.of(1,0,0))) );
          const dt = program_state.animation_delta_time;
          program_state.set_camera( desired_camera.map( (x,i) => Vec.from( program_state.camera_inverse[i] ).mix( x, .005*dt ) ) );
          ...
          const desired_camera = Mat4.inverse( this.bodies[0].drawn_location.times( Mat4.translation( [ 0,3,0 ] ) ) );
          const dt = program_state.animation_delta_time;
          program_state.set_camera( desired_camera.map( (x,i) => Vec.from( program_state.camera_inverse[i] ).mix( x, .005*dt ) ) );
      ...
      this.bodies[0].shape.draw(context,program_state,this.bodies[0].drawn_location, this.player_material.override(this.player_color));
      ...
      building_transformation_inner.post_multiply(Mat4.translation([10,0,0]));
       ...
       building_transformation_inner.post_multiply(Mat4.translation([-5,0,16]));
       draw_building_1( building_transformation_inner,12,8,12, orange, white);

       building_transformation_inner.post_multiply(Mat4.translation([5,0,23]));
       draw_building_2( building_transformation_inner,3,5,12, white, red);

      ////car/////
      //draw_car(_this.model_transform.times(Mat4.translation([0,0,0])));
      ...
      function draw_building_1(base_transformation, x,y, z, building_color, window_color){
        ...
        let m_body = new Body(_this.shapes.box, _this.materials.plastic.override( building_color), Vec.of(1,1,1));
       ...
       function draw_building_2(base_transformation, x, y, z, building_color, window_color){
        ...
        let m_body = new Body(_this.shapes.cylinder, _this.materials.plastic.override( building_color), Vec.of(1,1,1));
        m_body.shape.draw( context, program_state, body_transform, m_body.material);
...
//         _this.shapes.cylinder.draw( context, program_state, body_transform, _this.materials.plastic.override( building_color ) );
      ...
      function draw_road(){
        ...
        function draw_car(car_transform){
          ...
          _this.shapes.box.draw(context, program_state, base_transform, _this.materials.plastic.override( wheat ));
